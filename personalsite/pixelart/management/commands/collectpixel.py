    ...
    def handle(self, *args: Any, **options: Any) -> Optional[str]:
        ...
        for art_dir in new_art_dirs:
            ...
            if not "info.json" in contents:
                raise RuntimeError(
                    f"Folder {art_dir} does not contain an info.json file."
                )
            ...
            if not pixart_fnam in contents:
                raise RuntimeError(
                    f"Folder {art_dir} does not contain a {pixart_fnam} file."
                )
            if not thumb_fnam in contents:
                raise RuntimeError(
                    f"Folder {art_dir} does not contain a {thumb_fnam} file."
                )
            if not thumb_gs_fnam in contents:
                raise RuntimeError(
                    f"Folder {art_dir} does not contain a {thumb_gs_fnam} file."
                )

            success = True
            with open(os.path.join(art_dir, pixart_fnam), "r") as pixart_file, open(
                os.path.join(art_dir, thumb_gs_fnam), "rb"
            ) as empty_thumb_file, open(
                os.path.join(art_dir, thumb_fnam), "rb"
            ) as filled_thumb_file:
                try:
                    ...
                    success = False
                    print(
                        f'Could not create new art piece with slug "{slug}", one already exists in the database. Please edit info.json with a new slug and try again.'
                    )
            if success:
